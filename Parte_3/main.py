...
class Process:
    ...
    def __init__(self, name, data, max_time, operation, original_position):
        ...
        self.arrival_time = 0 # elapsed_seconds
        self.finish_time = 0
    ...
    def calculate_response_time(self):
        return (elapsed_seconds - self.arrival_time) if self.arrival_time > 0 else 0

    def calculate_waiting_time(self):
        return self.calculate_return_time() - self.calculate_response_time()


    def calculate_service_time(self):
        return self.calculate_return_time() - self.calculate_waiting_time()
...
def start_clock():
    global start_time, elapsed_seconds, clock_running
...
def update_elapsed_time():
    ...
    if clock_running:
        ...
        if len(finished_processes) == int(entry_num_processes.get()):
            stop_clock()
    lbl_global_clock.after(1000, update_elapsed_time)
...
def running_process():
    ...
    if not current_process and pending_processes:
        ...
        current_process.arrival_time = elapsed_seconds  # Set the arrival time when a process starts
    if current_process:
        if current_process.max_time > 0:
            current_process.max_time -= 1
        else:
            current_process.finish_time = elapsed_seconds  # Set the finish time when a process finishes
...
def generate_process():
    ...
    for i in range(n_processes):
        ...
        p.result = result
        p.arrival_time = elapsed_seconds  # Set the arrival time before adding to pending_processes
...
def generate_result_txt():
    ...
    if finished_processes:
        with open("results.txt", "w") as file:
            ...
            current_batch_R = 1
            # Generar tabla
            headers = ["PID", "Arrival Time", "Service Time", "Waiting Time", "Response Time", "Return Time"]
            rows = []
            for i, process in enumerate(finished_processes, start=1):
                rows.append([process.process_number, process.arrival_time, process.calculate_service_time(), process.calculate_waiting_time(), process.calculate_response_time(), process.calculate_return_time()])
                if i % 5 == 0 or i == len(finished_processes):
                    file.write("-" * 39 + f"Batch {current_batch_T}" + "-" * 39)
                    file.write("\n")
                    file.write(tabulate.tabulate(rows, headers=headers))
                    file.write("\n")
                    current_batch_T += 1
                    rows = []
            ...
            for i, process in enumerate(finished_processes, start=1):
                if i % 5 == 1 and i!= 1:
                    file.write("\n")
                if i % 5 == 1:
                    file.write(f"Batch {current_batch_R}\n")
                    current_batch_R += 1
